...
class DataAugmentationForDistorted(object):
    ...
    def __call__(self, image):
        c_x, c_y, ratio = random.uniform(-1, 1), random.uniform(-1, 1), random.uniform(0.15, 0.25)
        fisheye_grid = get_of_fisheye(224, 224, torch.tensor([c_x, c_y]), ratio)
        trans_img = self.transform(image)
        trans_img2 = copy.deepcopy(trans_img)

        return F.grid_sample(trans_img.unsqueeze(0), fisheye_grid, align_corners=True).squeeze(
            0), trans_img2, self.masked_position_generator()
...
def build_pretraining_dataset(args):
    transform = DataAugmentationForDistorted(args)
